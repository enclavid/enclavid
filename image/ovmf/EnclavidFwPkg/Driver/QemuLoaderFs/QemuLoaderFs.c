/** @file
  Serve the three measured blobs from fw_cfg, and only those.

  Derived from OvmfPkg/QemuKernelLoaderFsDxe (edk2-stable202602), which differs
  in two ways that compose into a hole:

  Upstream builds a blob from ANY fw_cfg file named under etc/boot/ -- the name
  comes from the host and the entry is created for it -- and fetches the
  contents before anything looks at the name. Then QemuKernelVerifyBlob returns
  EFI_SUCCESS for every name outside kernel/initrd/cmdline, so BlobVerifierLib,
  which halts the machine on a name it does not know, is never asked. The SEV
  hash table covers those three names, so anything else arrives measured by
  nobody while the launch digest stays what it was.

  A SECOND path builds blobs and is deliberately left alone:
  QemuKernelRegisterIgvmBlobs walks gEfiIgvmDataHobGuid HOBs and creates blobs
  named "shim" or "kernel" from them, calling no verifier. That is coherent
  rather than an oversight -- an IGVM file's pages are covered by the launch
  measurement, so their contents are already bound to it, and the two names are
  fixed in the code rather than taken from the host. It is also dormant here:
  nothing publishes those HOBs unless the guest is launched from an IGVM file,
  which this fleet is not. Gating it would guard a channel whose contents the
  digest already covers.

  Here the set of names is mKernelBlobItems, which is a compile-time constant:
  a directory entry can fill in a size and a selector for a name already in it,
  and a name that is not in it is skipped before the contents are read. An
  unknown name is not rejected late, it never becomes a blob. The verifier is
  fail-closed as a second answer to the same question.
...
STATIC
BOOLEAN
QemuKernelBlobNameIsKnown (
  IN CONST CHAR16  *Name
  )
{
  UINTN  Idx;

  for (Idx = 0; Idx < ARRAY_SIZE (mKernelBlobItems); Idx++) {
    if (StrCmp (Name, mKernelBlobItems[Idx].Name) == 0) {
      return TRUE;
    }
  }
  ...
  )
{
  ...
  //
  // Refuse what cannot be checked. Upstream returns EFI_SUCCESS here, which
  // means an unrecognised name is waved past the verifier rather than stopped
  // by it. Nothing reaches this with an unknown name any more -- the fetch loop
  // drops those -- so this is the second of two answers, not the only one.
  //
  if (!QemuKernelBlobNameIsKnown (FileName)) {
    DEBUG ((DEBUG_ERROR, "%a: refusing unknown blob \"%s\"\n", __func__, FileName));
  ...
  )
{
  ...
  for (Idx = 0; Idx < Count; ++Idx) {
    ...
    //
    // The host chose that name. Serve it only if it is one this build already
    // knows, so that a blob the SEV hash table does not cover is never read in
    // the first place.
    //
    if (!QemuKernelBlobNameIsKnown (Items.Name)) {
      DEBUG ((
        DEBUG_WARN,
        "%a: ignoring unknown named blob \"%s\"\n",
        __func__,
        Items.Name
        ));
      continue;
