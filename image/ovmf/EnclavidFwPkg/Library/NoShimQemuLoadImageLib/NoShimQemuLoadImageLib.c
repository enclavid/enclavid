/** @file
  QemuLoadImageLib that loads the kernel and nothing else.

  Derived from OvmfPkg/Library/GenericQemuLoadImageLib (edk2-stable202602).
  Upstream calls gBS->LoadImage on a "shim" file BEFORE it tries the kernel and
  falls back to the kernel only on EFI_NOT_FOUND. The loader filesystem it reads
  from publishes whatever the host names under fw_cfg's etc/boot/, while the SEV
  hash table covers only kernel, initrd and cmdline, so a host-supplied "shim"
  runs in place of the measured kernel and the launch digest does not move.

  This copy never names a shim, so there is nothing for the host to be served
  into. The diff against the upstream file is the whole of the change.
