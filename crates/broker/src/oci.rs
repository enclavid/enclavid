...
//! verbatim. The broker authenticates to no registry on its own.
//!
...
use broker_protocol::{PullRequest, PullResponse};
...
/// POST /oci/pull
pub async fn pull(body: Bytes) -> Result<Vec<u8>, BrokerError> {
...
fn classify_oci_error(e: oci_client::errors::OciDistributionError) -> BrokerError {
    let msg = format!("{e:?}");
    if msg.contains("MANIFEST_UNKNOWN") || msg.contains("code: 404") || msg.contains("404") {
        BrokerError::NotFound
    } else {
        BrokerError::Internal(format!("pull: {e}"))
    }
}
...
fn parse_ref(policy_ref: &str) -> Result<Reference, BrokerError> {
    let reference = Reference::try_from(policy_ref)
        .map_err(|e| BrokerError::BadRequest(format!("invalid policy_ref: {e}")))?;
...
fn build_auth(registry_auth: &[u8]) -> Result<RegistryAuth, BrokerError> {
    if registry_auth.is_empty() {
        return Ok(RegistryAuth::Anonymous);
    }
    let s = std::str::from_utf8(registry_auth)
        .map_err(|_| BrokerError::BadRequest("registry_auth not utf-8".to_string()))?
        .trim();
