//! Concrete domain stores wrapping host-side services.
//!
//! - `SessionStore` — per-session typed fields (status, metadata,
//!   state) plus the disclosure list, via the `SessionStore`
...
//!
//! Engine never talks to broker-client directly. It accumulates pending
...
//! disclosures.
//!
//! All broker-client return values are wrapped in `Untrusted<T>`. Even
