...
//! client `Authorization` gate (`HATCH_AUTH=oidc` JWKS verification, or
...
/// Request-body cap for the hatch. Axum defaults to 2 MB, but a
...
async fn main() -> anyhow::Result<()> {
    ...
    let redis_url = required_env("HATCH_REDIS_URL")?;
    ...
    let cache_dir = PathBuf::from(required_env("HATCH_CACHE_DIR")?);
