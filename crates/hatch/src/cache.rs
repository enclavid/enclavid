...
//! `GET /cache/{key}` reads it back (or 404). The hatch never
...
//! compatibility) is enforced TEE-side above this transport; the hatch
//! only moves opaque blobs.
//!
//! Backend is an [`ObjectStore`]: today a local filesystem (per-instance
//! disk that survives a TEE / hatch restart, so a cold start re-uses
...
//! (`hatch-client::CacheStore`), so it is pure lowercase hex — validated
...
fn object_path(key: &str) -> Result<ObjPath, HatchError> {
    if key.is_empty() || key.len() > MAX_KEY_LEN {
        return Err(HatchError::BadRequest("cache key length".to_string()));
    }
...
pub async fn store(
    ...
    state
        ...
        .put(&path, body.into())
        .await
        .map_err(|e| HatchError::Internal(format!("cache write: {e}")))?;
    Ok(StatusCode::OK)
}
    ...
    match state.cache_store.get(&path).await {
        Ok(res) => {
            let bytes = res
                .bytes()
                .await
                .map_err(|e| HatchError::Internal(format!("cache read: {e}")))?;
            Ok((StatusCode::OK, bytes).into_response())
        }
        Err(object_store::Error::NotFound { .. }) => Err(HatchError::NotFound),
        Err(e) => Err(HatchError::Internal(format!("cache get: {e}"))),
    }
}
