...
enum PolicyCommand {
    ...
    /// Encrypt a wasm policy with a client_policy_key.
    Encrypt {
    ...
    Push {
        ...
        /// Path to the policy manifest JSON file. Bundled as the
        /// second OCI layer (plain, not encrypted). Point to a
        /// missing file if the policy has no host-visible UI
        /// (decision-only stubs).
    ...
    match cli.command {
        ...
        Commands::Policy { command } => match command {
            ...
            PolicyCommand::Push {
                ...
                auth,
                manifest,
            } => commands::policy::push::run(artifact, reference, auth, manifest).await,
